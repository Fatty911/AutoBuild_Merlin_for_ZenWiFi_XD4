...
# （上游缺陷：RT-AX55 无 MSSID_PRELINK、无 BT_CONN（XD4 定义已移除 BT_CONN），
#   这些路径从未被构建暴露；XD4 是 386 分支首个 MSSID_PRELINK=y 机型）。
